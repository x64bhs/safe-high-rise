...
async def get_flood_risk(lat: float, lon: float, precipitation: float):
    """
    Comprehensive flood risk assessment based on multiple factors:
    1. Elevation (low-lying areas more prone to flooding)
    2. Proximity to water bodies (coastlines, rivers, lakes)
    3. Precipitation patterns
    4. Topographical drainage (land-locking)
    
    Returns detailed flood risk profile.
    """
    
    # Factor 1: Get elevation data from Open-Meteo
    elevation = await get_elevation(lat, lon)
    
    # Factor 2: Calculate proximity to nearest coastline
    coastal_proximity = calculate_coastal_proximity(lat, lon)
    
    # Factor 3: Assess precipitation risk
    precip_risk_score = assess_precipitation_risk(precipitation)
    
    # Factor 4: Determine if area is land-locked or has drainage issues
    drainage_score = assess_drainage(lat, lon, elevation)
    
    # Combine all factors into comprehensive risk assessment
...
def calculate_coastal_proximity(lat: float, lon: float):
    ...
    if is_likely_coastal(lat, lon):
        return 10  # Assume 10km if coastal pattern detected
    
    # Otherwise return calculated distance (capped at 500km for inland areas)
    return min(min_distance, 500)
...
def calculate_flood_risk(elevation, coastal_proximity, precip_risk, drainage_score, precipitation):
    ...
    elevation_score = 0
    if elevation < 0: elevation_score = 100 # Below sea level
    elif elevation < 2: elevation_score = 95
    elif elevation < 5: elevation_score = 85
    elif elevation < 10: elevation_score = 70
    elif elevation < 25: elevation_score = 45
    elif elevation < 50: elevation_score = 25
