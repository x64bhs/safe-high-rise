...
async def analyze_location(data: LocationData):
    ...
    # 1.5 Auto-Description
    location_info = await get_location_info(data.latitude, data.longitude, weather_profile, seismic_profile)
    
    # 2. Generate Logic-Driven Design
    design_result = generate_architectural_design(weather_profile, seismic_profile)
    ...
    return {
        ...
        "profile": {
            ...
            "flood_risk": "Low (Estimated)", 
            "precipitation": f"{weather_profile['precipitation']} mm"
        },
