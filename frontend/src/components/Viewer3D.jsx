                    ...
                    <group key={floor.id} position={[0, floor.y, 0]} rotation={[0, floor.rotation, 0]} scale={[floor.scale, 1, floor.scale]}>
                        ...
                        {isCutaway && (
                            <group>
                                ...
                                {(() => {
                                    const mat = materialRec;
                                    let layerColor = "#64748b";
                                    let layerOpacity = 0.4;
                                    let layerEmissive = "#000000";

                                    if (mat.includes("self-healing") || mat.includes("graphene")) {
                                        layerColor = "#6366f1"; // Indigo for smart materials
                                        layerEmissive = "#312e81";
                                    } else if (mat.includes("living") || mat.includes("bio") || mat.includes("algae")) {
                                        layerColor = "#22c55e"; // Green for bio materials
                                        layerEmissive = "#14532d";
                                    } else if (mat.includes("carbon") || mat.includes("titanium")) {
                                        layerColor = "#1e293b"; // Dark for high-tech
                                    }
                        ...
                        {!isCutaway && i % 5 === 0 && i > 0 && (
                            <group>
                                {[0, 90, 180, 270].map((angle, idx) => {
                                    const radius = geoParams.type === 'cylinder' ? 5.2 * floor.scale :
                                        geoParams.type === 'hexagon' ? 5.2 * floor.scale :
                                            geoParams.type === 'triangle' ? 6.2 * floor.scale : 5.5;
                        ...
                        <mesh position={[0, 1, 0]}>
                            ...
                            <meshPhysicalMaterial
                                color={(() => {
                                    if (simulationMode === 'fire') return "#fca5a5";
                                    const mat = materialRec;
                                    if (mat.includes("living") || mat.includes("forest") || mat.includes("bio")) return "#86efac";
                                    if (mat.includes("smart") || mat.includes("kinetic")) return "#c7d2fe";
                                    return "#bae6fd";
                                })()}
                                transmission={isCutaway ? 0.7 : 0.3}
                                opacity={simulationMode === 'fire' ? 0.3 : isCutaway ? 0.4 : 0.6}
                                transparent
                                roughness={0.1}
                                thickness={1.5}
                                clippingPlanes={clipPlanes}
                                side={THREE.DoubleSide}
                                emissive={materialRec.includes("smart") ? "#4f46e5" : "#000000"}
                                emissiveIntensity={materialRec.includes("smart") ? 0.1 : 0}
                            />
                        </mesh>
                        ...
                        {!isCutaway && i % 2 === 0 && (
                            <group>
                                {(() => {
                                    const frames = [];
                                    const segments = geoParams.type === 'cylinder' ? 8 :
                                        geoParams.type === 'hexagon' ? 6 :
                                            geoParams.type === 'triangle' ? 3 : 4;
